...
std::vector<unsigned short> Parser::BR_2NNN(unsigned short Index) {
	unsigned short ljump = word & 0x0FFF;
	isFunction = true;
	return std::vector<unsigned short>{ljump};
}
...
std::map<unsigned short, BasicBlock*> Parser::LinkBlocks(std::map<unsigned short, BasicBlock*> blocksWithEntryAddresses) {
	for (auto const&blockMap : blocksWithEntryAddresses) {
		BasicBlock* block = blockMap.second;
		unsigned short prevKey = 0;
		for (unsigned short adr : block->entryAddresses) {
			for (auto const&[key, val] : blocksWithEntryAddresses) {
				if (adr >= prevKey && adr < key) {
					block->entryBlocks.push_back(blocksWithEntryAddresses[prevKey]);
					block->entryCount++;
					blocksWithEntryAddresses[prevKey]->exitBlocks.push_back(block);
					blocksWithEntryAddresses[prevKey]->exitCount++;
					break;
				}
				prevKey = key;
			}
	...
	for (auto const&blockMap : blocksWithEntryAddresses) {
		...
		if (block->exitCount == 1) {
			BasicBlock* linkedBlock = block->exitBlocks[0];
...
std::vector<BasicBlock*> Parser::FillBlocks(std::map<unsigned short, BasicBlock*> emptyBlocks, unsigned short dataPartition) {
	...
	blockAddresses.push_back(programLength +2); //just so blockIndex+1 doesnt overflow
	BasicBlock* currentBlock = emptyBlocks[0];
